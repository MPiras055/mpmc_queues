...
{
    struct __lfring * q = (struct __lfring *) ring; //reference to the right ring
    size_t tidx, half = lfring_pow2(order), n = half * 2;
    lfatomic_t tail, entry, ecycle, tcycle;

    eidx ^= (n - 1);    //xor in order not to trash the sequence

    while (1) {
        //get an index
        tail = atomic_fetch_add_explicit(&q->tail, 1, memory_order_acq_rel);
        //check closure of the queue
        if (tail & __LFRING_CLOSED)
            return false;

        //compute the cycle
        tcycle = (tail << 1) | (2 * n - 1); //still assert overflow
        tidx = __lfring_map(tail, order, n);    //map the tail to the index (modulo mapping to pow2)
        entry = atomic_load_explicit(&q->array[tidx], memory_order_acquire);    //load the entry
        retry:
        ecycle = entry | (2 * n - 1);   //or in order to avoid overflow

        if (__lfring_cmp(ecycle, <, tcycle) &&  //compare 2 unsigned
            ((entry == ecycle) ||
            ((entry == (ecycle ^ n)) &&
...
void lfring_open(struct lfring *ring) {
    ...
    atomic_store_explicit(
        &q->tail,atomic_load_explicit(&q->head,memory_order_acquire),
